...
*   **City:** PixiJS v8 (Canvas) for isometric performance; implemented in `src/games/city/CityScene.js` with PNG assets under `public/assets/city/` resolved against the active base path, viewport culling for ground tiles, and deterministic grass variants (`grass_01`-`grass_03`) for variety.
